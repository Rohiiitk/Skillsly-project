...
const MessageInvitation = ({ user, role, date, time }) => (
  <div className="relative p-4 md:p-5 rounded-xl text-sm max-w-sm ml-auto mr-auto bg-gray-700">
    ...
    <div className="absolute right-3 top-3 flex space-x-2 text-gray-400">
      <button className="p-1 rounded-full hover:bg-black/50 hover:text-teal-400 transition-colors"><Check className="w-5 h-5" /></button>
      <button className="p-1 rounded-full hover:bg-black/50 hover:text-red-500 transition-colors"><X className="w-5 h-5" /></button>
    </div>
  </div>
);
  ...
  <div className="p-6 md:p-10 w-full h-full flex flex-col">
    <div className="flex-grow space-y-4 mb-8">
      {messageInvitations.map(msg => (
        <MessageInvitation key={msg.id} {...msg} />
      ))}
      <div className="flex justify-end pr-0 md:pr-10">
        <div className="w-56 p-2 bg-[#1a1a1a] rounded-xl flex flex-col items-end">
          ...
          <button className="flex items-center justify-center bg-teal-600 hover:bg-teal-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors">
    ...
    <div className="flex justify-center space-x-4 pt-4 border-t border-gray-800">
      <button className="bg-[#1a1a1a] text-white font-semibold py-3 px-8 rounded-lg hover:bg-gray-700 transition-colors">Real Time Chat</button>
      <button className="bg-[#1a1a1a] text-white font-semibold py-3 px-8 rounded-lg hover:bg-gray-700 transition-colors">Schedule session</button>
    </div>
  </div>
);
